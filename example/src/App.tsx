...
const colors = {
  normal: 'rgba(62, 130, 247, 1)',
  warn: 'rgba(255, 241, 118, 1)',
  fail: 'rgba(239, 83, 80, 1)',
  success: 'rgba(38, 166, 154, 1)',
};
...
const App: FunctionComponent = () => {
  ...
  const authenticate = async () => {
    ...
    try {
      ...
      const result = await Fingerprint.authenticate((warning) => {
        setPhase('warn');
  ...
  useEffect(() => {
    authenticate();
    const handleResume = async (state: AppStateStatus) => {
      try {
        if (state === 'active' && (await Fingerprint.isAuthenticationCanceled()) === true) {
          authenticate();
        }
      } catch (z) {
        console.error(z);
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Text style={styles.welcome}>
          {phase === 'normal' && 'Touch the fingerprint sensor.'}
          {phase === 'warn' && 'Try again.'}
          {phase === 'fail' && 'Authentication failed.'}
          {phase === 'success' && 'Authentication succeeded.'}
        </Text>
        <Text>{message}</Text>
      </View>
